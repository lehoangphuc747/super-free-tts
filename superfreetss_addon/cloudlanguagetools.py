...
class CloudLanguageTools():
    ...
    def get_request_headers(self):
        if self.config.use_vocabai_api:
            ...
            return {
                ...
                'client': 'superfreetss', 
                'client_version': version.ANKI_SUPER_FREE_TTS_VERSION,
                'User-Agent': f'anki-super-free-tts/{version.ANKI_SUPER_FREE_TTS_VERSION}'}
    ...
    def _compute_hmac_signature(self, email, client_uuid, machine_id):
        import hashlib
        import hmac
    ...
    def build_trial_key_request_data(self, email, password, client_uuid):
        try:
            # Note: machineid is a custom requirement, fallback to client_uuid if missing
            import machineid
            machine_id = machineid.id()
        except:
            machine_id = client_uuid
        
        hmac_signature = self._compute_hmac_signature(email, client_uuid, machine_id)
