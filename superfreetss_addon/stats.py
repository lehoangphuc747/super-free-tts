...
class StatsGlobal:
    ...
    def __init__(self, anki_utils, user_uuid, user_properties, first_install, superfreetss_pro: bool):
        ...
        self.superfreetss_pro = superfreetss_pro
    ...
    def publish_event(self, 
        ...
        if context == constants_events.EventContext.addon:
            event_properties['superfreetss_addon_version'] = version.ANKI_SUPER_FREE_TTS_VERSION
            event_properties['anki_version'] = anki.version
            event_properties['$set'] = {
                'anki_version': anki.version,
                'superfreetss_addon_version': version.ANKI_SUPER_FREE_TTS_VERSION,
                'superfreetss_addon_user': True,
    ...
    def init_load(self):
        ...
        if not self.superfreetss_pro: # don't load them if we are in pro mode
    ...
    def load_background(self):
        ...
        if not self.superfreetss_pro: # don't report them if we are in pro mode
...
def event_global(event: constants_events.Event):
    if hasattr(sys, '_superfreetss_stats_global'):
        sys._superfreetss_stats_global.publish(constants_events.EventContext.addon, event, None, {})

def send_event(context: constants_events.EventContext, event: constants_events.Event, event_mode: constants_events.EventMode,
               event_properties: dict):
    if hasattr(sys, '_superfreetss_stats_global'):
        sys._superfreetss_stats_global.publish(context, event, event_mode, event_properties)
...
def send_event_bg(context: constants_events.EventContext, event: constants_events.Event, event_mode: constants_events.EventMode,
    ...
    if hasattr(sys, '_superfreetss_stats_global'):
        sys._superfreetss_stats_global.publish_event(context, event, event_mode, event_properties)
...
def feature_flag_enabled(flag_key: str):
    ...
    if hasattr(sys, '_superfreetss_stats_global'):
        return sys._superfreetss_stats_global.get_feature_flag_enabled(flag_key)
    return False

def feature_flag_value(flag_key: str) -> str:
    # check if stats are enabled, default to control
    if hasattr(sys, '_superfreetss_stats_global'):
        if sys._superfreetss_stats_global.get_feature_flag_enabled(flag_key):
            return sys._superfreetss_stats_global.get_feature_flag_value(flag_key)
