...
class SuperFreeTTS():
    ...
    def editor_note_add_audio(self, 
        ...
        if batch.target.insert_location == config_models.InsertLocation.CURSOR_LOCATION:
            raise errors.SuperFreeTTSError('inserting at cursor location not yet supported')
    ...
    def expand_advanced_template(self, note, source_template):
        raise errors.SuperFreeTTSError("Tính năng Advanced Template (Python code) không được hỗ trợ trong bản Lite để đảm bảo bảo mật.")
    ...
    def card_template_has_tts_tag(self, note, side, card_ord):
        ...
        # Hỗ trợ cả hypertts_preset (cũ) và superfreetss_preset (mới) cho backward compatibility
        m = re.match(r'.*{{tts.*(?:hypertts_preset|superfreetss_preset)=([^\s]+).*}}.*', side_template)
        if m != None:
