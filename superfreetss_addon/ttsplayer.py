...
class AnkiSuperFreeTTSPlayer(aqt.tts.TTSProcessPlayer):
    ...
    def get_available_voices(self) -> List[aqt.tts.TTSVoice]:
        ...
        for audio_language in languages.AudioLanguage:
            ...
            if anki.utils.point_version() == 58: # this regression only concerns Anki 2.1.58
                ...
                voices.append(aqt.tts.TTSVoice(name='HyperTTS', lang=language_name, available=True))  # backward compat
            else:
                voices.append(aqt.tts.TTSVoice(name=constants.TTS_TAG_VOICE, lang=language_name))
                voices.append(aqt.tts.TTSVoice(name='HyperTTS', lang=language_name))  # backward compat
    ...
    def _play(self, tag: anki.sound.AVTag):
        ...
        # Hỗ trợ cả HyperTTS (cũ) và SuperFreeTTS (mới) cho backward compatibility
        if constants.TTS_TAG_VOICE not in tag.voices and 'HyperTTS' not in tag.voices:
